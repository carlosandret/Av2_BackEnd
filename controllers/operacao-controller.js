...
//     operacao.validate()
//     /** Pega o id gerado ao criar a operação*/
//     const idGerado = operacao.create()
//     // Atualiza o objeto com o ID gerado
//     operacao.data.id = idGerado;
...
//         res.redirect('/operacao/operacoes') // redirecionar para a página de listagem de operações após salvar
//     }
// }
...
// Função para pegar o id da operação que será editada
exports.idParaEdicao = function (req, res) {
    const idOperacao = Number(req.body.idOperacao);
    // console.log(idOperacao)
    res.redirect(`/operacao/editar_operacao?id=${idOperacao}`);
    // res.render('pages/editar_operacao', { idOperacao });
}

exports.update = function (req, res) {
    const idOperacao = Number(req.body.idOperacao || req.query.id);

    console.log("Id operação update:",idOperacao)
